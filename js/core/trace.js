...
window.addEventListener('error', (e) => {
  // Страницы прерывают свой модуль исключением, когда уводят участника
  // на нужный экран (доступ закрыт, анкета уже подана). Это штатный ход,
  // а не поломка — иначе диагностика показывала бы ошибку после каждого
  // такого перехода.
  if (/^переход |^нет доступа$/.test(e.message || '')) {
    trace('доступ: модуль остановлен', e.message);
    return;
  }
